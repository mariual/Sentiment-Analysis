...
class MultiHeadedAttention(nn.Module):
    ...
    def merge_heads(self, tensor):
        ...
        batch_size, num_heads, seq_length, head_size = tensor.size()
        tensor = tensor.permute(0, 2, 1, 3).contiguous()
        return tensor.view(batch_size, seq_length, num_heads * head_size)
...
class Transformer(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        if mask is not None:
            mask = torch.cat([torch.ones(B, 1, dtype=mask.dtype, device=mask.device), mask], dim=1)

        for layer in self.transformer:
            x = layer(x, mask)
        
        cls_representation = x[:, 0]
        output = self.mlp_head(cls_representation)
        return output
